...
def ex2():
  ...
  pareto_expected_sim = sim.mean(sims, pareto)
  erlang_expected_sim = sim.mean(sims, erlang)
  weibull_expected_sim = sim.mean(sims, weibull)
...
def ex3():
  ...
  start = time()
  exp_val_sim = sim.mean(sims, ex3b)
  time_sim = time() - start

  start = time()
  exp_val_optimized = sim.mean(sims, ex3b_optimized)
